    ...
    <div className="relative min-h-screen md:flex flex-col items-center pt-8 max-w-60 w-full border-r border-borderColor text-sm">
      ...
      <div className="w-full">
        {ownerMenuLinks.map((link, index) => (
          ...
          >
            <img src={link.path === location.pathname ? link.coloredIcon : link.icon} alt="menu icon" className="h-5 w-5" />
            <span className="max-md:hidden">{link.name}</span>
            {link.path === location.pathname && <div className="bg-primary w-1.5 h-8 rounded-r absolute right-0"></div>}
          </NavLink>
        ))}
      </div>
    </div>
